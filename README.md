...
* **[Dependency Checker](#dependency-checker)**
	...
	* Commands: Tables
		* [`mpmd:dependencycheck`](#command-mpmddependencychecker)
		* [`mpmd:dependencycheck:verify`](#command-mpmddependencycheckerverify)
		* [`mpmd:dependencycheck:configured`](#command-mpmddependencycheckerconfigured)
	* Commands: Graphs
		* [`mpmd:dependencycheck:graph:module`](#command-mpmddependencycheckergraphmodule)
		* [`mpmd:dependencycheck:graph:class`](#command-mpmddependencycheckergraphclass)
		* [`mpmd:dependencycheck:graph:configured`](#command-mpmddependencycheckergraphconfigured)
...
### Command: `mpmd:dependencycheck:verify`
...
### Command: `mpmd:dependencycheck:configured`

This command returns a list of all **configured** dependencies (taken from config xml). This (and the corresponding `mpmd:dependencycheck:graph:configured`) is the only command that does not analyze any files but only reads the dependencies from the configuration.
...
### Command: `mpmd:dependencycheck:graph:module`
...
### Command: `mpmd:dependencycheck:graph:class`

While the previous command shows you a higher level view on modules only, `mpmd:dependencycheck:graph:class` will drill down into individual classes and optionally group them by module. Consider this graph "zooming in" into the modules in order to find out what classes are responsible for the dependency.
...
### Command: `mpmd:dependencycheck:graph:configured`

This command will create a graph from the **configured** dependencies. The syntax is the same used in `mpmd:dependencycheck:configured`:
